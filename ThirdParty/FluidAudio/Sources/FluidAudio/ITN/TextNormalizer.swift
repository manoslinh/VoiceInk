...
public final class TextNormalizer: Sendable {
    ...
    public func normalizeSentence(_ input: String) -> String {
        let cleaned = Self.stripSpecialTokens(input)
        guard isNativeAvailable else {
            return cleaned
        }

        let filtered = filterAmbiguousWords(in: cleaned)
        return callNormalizeSentence(filtered)
    }

    /// Removes decoder special tokens (e.g. `<unk>`, `<pad>`, `<s>`) that can
    /// surface as literal text when the model emits an out-of-vocab/blank id —
    /// notably the unknown token, whose ASCII glyphs pass any script filter.
    /// Collapses the whitespace left behind so " word <unk> word " → "word word".
    static func stripSpecialTokens(_ input: String) -> String {
        let tokens = ["<unk>", "<pad>", "<s>", "</s>", "<blank>"]
        var output = input
        for token in tokens {
            output = output.replacingOccurrences(of: token, with: " ")
        }
        let collapsed = output.replacingOccurrences(
