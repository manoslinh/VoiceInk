...
internal struct TokenLanguageFilter: Sendable {
    ...
    static func filterTopK(
        ...
        for idx in 0..<count {
            ...
            guard allowedScripts.contains(where: { matches(tokenText, script: $0) }) else { continue }
    ...
    /// Thin single-script wrapper around the `allowedScripts:` overload, kept
    /// so external callers that pass a single `preferredScript` keep compiling
    ...
    static func filterTopK(
        ...
        filterTopK(
            ...
            allowedScripts: [preferredScript]
        )
    }

    /// Returns true when `text` matches any of the given `scripts`.
    static func matchesAny(_ text: String, scripts: Set<Script>) -> Bool {
        scripts.contains(where: { matches(text, script: $0) })
    }
}
