...
public actor AsrManager {
    ...
    internal func tdtDecodeWithTimings(
        ...
        case .v2, .tdtCtc110m:
            if language != nil || !allowedScripts.isEmpty {
                logger.debug(
                    "Ignoring `language` / `allowedScripts` hint: script filtering requires the v3 joint decoder; loaded model is \(models.version)."
                )
            }
        ...
        case .tdtJa:
            ...
            if language != nil || !allowedScripts.isEmpty {
                logger.debug(
                    "Ignoring `language` / `allowedScripts` hint for tdtJa: TokenLanguageFilter currently supports Latin/Cyrillic only; Japanese output is always kept."
                )
            }
            let decoder = TdtDecoderV3(config: adaptedConfig)
            return try await decoder.decodeWithTimings(
    ...
    public func transcribe(
        ...
        return try await transcribe(
            audioFloatArray, decoderState: &decoderState, language: language, allowedScripts: allowedScripts)
    }
        ...
        if config.streamingEnabled {
            ...
            if estimatedSamples > config.streamingThreshold {
                return try await transcribeDiskBacked(
                    url, decoderState: &decoderState, language: language, allowedScripts: allowedScripts)
            }
        }
    ...
    public func transcribe(
        ...
        do {
            let result = try await transcribeWithState(
                audioSamples, decoderState: &decoderState, language: language, allowedScripts: allowedScripts)
