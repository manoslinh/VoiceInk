...
internal struct TdtDecoderV3: Sendable {
    ...
    func decodeWithTimings(
        ...
        // Resolve the active script set. When `allowedScripts` is empty we
        // derive it from `language` exactly as before, preserving the legacy
        // single-script behavior. A non-empty `allowedScripts` takes over and
        // lets multiple scripts pass the per-token filter in one decode pass.
        let effectiveScripts: Set<Script> =
            allowedScripts.isEmpty ? (language.map { [$0.script] } ?? []) : allowedScripts

        // Script-filtering consumes top-K; skip the extraction when no scripts
        // are active (default path), so v3 joint runs don't pay for K-length
        // array allocations they'll never use.
        let needsTopK = !effectiveScripts.isEmpty
        ...
        while activeMask {
            ...
            Self.tokenLanguageFilter(
                ...
                allowedScripts: effectiveScripts,
    ...
    private static func tokenLanguageFilter(
        ...
        guard !allowedScripts.isEmpty,
            ...
            !TokenLanguageFilter.matchesAny(tokenText, scripts: allowedScripts),
